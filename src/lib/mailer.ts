...
<body style="margin:0;padding:0;background:#f9f9f9;font-family:'Helvetica Neue',Arial,sans-serif;">
  <div style="max-width:600px;margin:40px auto;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    <div style="background:#111;padding:32px 40px;">
      <h1 style="margin:0;color:#fff;font-size:22px;letter-spacing:1px;">Shoe World</h1>
    ...
    <div style="background:#f4f4f4;padding:24px 40px;text-align:center;">
      <p style="margin:0;color:#999;font-size:12px;">Shoe World · Premium Pakistani Footwear</p>
    </div>
  </div>
</body></html>`;

  await transporter.sendMail({
    from: `"Shoe World" <${process.env.SMTP_USER}>`,
    to: data.toEmail,
    subject: `Order Confirmed #${data.orderId} — Shoe World`,
    html,
  });
}
...
<body style="margin:0;padding:0;background:#f9f9f9;font-family:'Helvetica Neue',Arial,sans-serif;">
  ...
  await transporter.sendMail({
    from: `"Shoe World" <${process.env.SMTP_USER}>`,
    to: email,
    subject: "Verify Your Email — Shoe World",
    html,
  });
}
...
<body style="margin:0;padding:0;background:#f9f9f9;font-family:'Helvetica Neue',Arial,sans-serif;">
  ...
  await transporter.sendMail({
    from: `"Shoe World" <${process.env.SMTP_USER}>`,
    to: email,
    subject: "Reset Your Password — Shoe World",
    html,
  });
}
...
<body style="margin:0;padding:0;background:#f9f9f9;font-family:'Helvetica Neue',Arial,sans-serif;">
  ...
  await transporter.sendMail({
    from: `"Shoe World" <${process.env.SMTP_USER}>`,
    to: email,
    subject: "Your Verification Code — Shoe World",
    html,
